...
* Fix: Child themes using `growgoal_body_open()` no longer working ([#278](https://github.com/elementor/hello-theme/issues/278))
...
* Tweak: Add `growgoal_enable_skip_link` filter to enable/disable the skip link
* Tweak: Add `growgoal_skip_link_url` filter to change skip link URL
...
* Tweak: Delete the `growgoal_load_textdomain` filter hook
...
* Tweak: Remove header tags with `growgoal_page_title` filter
...
* New: Added `growgoal_page_title` filter for show\hide page title
* New: Added `growgoal_enqueue_theme_style` filter for enqueue theme-specific style
...
= 2.0.2 - 2019-05-13 =
* Tweak: Added `growgoal_content_width` filter, as per WordPress best practice
...
* Tweak: Change functions.php methods names prefix from `growgoal_theme_` to `growgoal_`
