...
add_action( 'elementor/init', 'growgoal_settings_init' );

function growgoal_settings_init() {
 ...
 */
function growgoal_get_setting( $setting_id ) {
	global $growgoal_settings;

	$return = '';

	if ( ! isset( $growgoal_settings['kit_settings'] ) ) {
		$kit = \Elementor\Plugin::$instance->kits_manager->get_active_kit();
		$growgoal_settings['kit_settings'] = $kit->get_settings();
	}

	if ( isset( $growgoal_settings['kit_settings'][ $setting_id ] ) ) {
		$return = $growgoal_settings['kit_settings'][ $setting_id ];
	}

	return apply_filters( 'growgoal_' . $setting_id, $return );
}
...
function hello_show_or_hide( $setting_id ) {
	return ( 'yes' === growgoal_get_setting( $setting_id ) ? 'show' : 'hide' );
}
...
function hello_get_header_layout_class() {
	...
	$header_layout = growgoal_get_setting( 'hello_header_layout' );
	...
	$header_width = growgoal_get_setting( 'hello_header_width' );
	if ( 'full-width' === $header_width ) {
		$layout_classes[] = 'header-full-width';
	}

	$header_menu_dropdown = growgoal_get_setting( 'hello_header_menu_dropdown' );
	...
	$hello_header_menu_layout = growgoal_get_setting( 'hello_header_menu_layout' );
...
function hello_get_footer_layout_class() {
	$footer_layout = growgoal_get_setting( 'hello_footer_layout' );
	...
	$footer_width = growgoal_get_setting( 'hello_footer_width' );

	if ( 'full-width' === $footer_width ) {
		$layout_classes[] = 'footer-full-width';
	}

	if ( growgoal_get_setting( 'hello_footer_copyright_display' ) && '' !== growgoal_get_setting( 'hello_footer_copyright_text' ) ) {
...
function hello_get_header_display() {
	...
	return (
		$is_editor
		|| growgoal_get_setting( 'hello_header_logo_display' )
		|| growgoal_get_setting( 'hello_header_tagline_display' )
		|| growgoal_get_setting( 'hello_header_menu_display' )
	);
}
...
function hello_get_footer_display() {
	...
	return (
		$is_editor
		|| growgoal_get_setting( 'hello_footer_logo_display' )
		|| growgoal_get_setting( 'hello_footer_tagline_display' )
		|| growgoal_get_setting( 'hello_footer_menu_display' )
		|| growgoal_get_setting( 'hello_footer_copyright_display' )
	);
}
...
add_action( 'elementor/experiments/default-features-registered', function( \Elementor\Core\Experiments\Manager $experiments_manager ) {
	$experiments_manager->add_feature( [
		...
		'title' => esc_html__( 'Header & Footer', 'growgoal' ),
		'tag' => esc_html__( 'Hello Theme', 'growgoal' ),
		'description' => sprintf(
			'%1$s <a href="%2$s" target="_blank">%3$s</a>',
			esc_html__( 'Customize and style the builtin Hello Theme’s cross-site header & footer from the Elementor "Site Settings" panel.', 'growgoal' ),
			'https://go.elementor.com/wp-dash-header-footer',
			esc_html__( 'Learn More', 'growgoal' )
		),
