...
class Mailchimp_API {
    ...
    public static function init() {
        add_action( 'wp_ajax_edubin_mailchimp_subscription', [ __CLASS__, 'mailchimp_subscribe_through_ajax' ] );
        add_action( 'wp_ajax_nopriv_edubin_mailchimp_subscription', [ __CLASS__, 'mailchimp_subscribe_through_ajax' ] );
    }
    ...
    public static function mailchimp_subscribe_through_ajax() {
        ...
        $merge_fields = array(
            'FNAME' => ! empty( $settings['edubin_mailchimp_firstname'] ) ? $settings['edubin_mailchimp_firstname'] : '',
            'LNAME' => ! empty( $settings['edubin_mailchimp_lastname'] ) ? $settings['edubin_mailchimp_lastname'] : '',
        );

        $result = json_decode( self::mailchimp_subscribe( $settings['edubin_mailchimp_email'], 'subscribed', $list_id, $api_key, $merge_fields, @$settings['edubin_mailchimp_phone'] ) );

        if ( 400 === $result->status ) :
            _e( 'Error', 'edubin-core' );
        elseif ( 'subscribed' === $result->status ) :
            _e( 'Congratulation, You have subscribed successfully!', 'edubin-core' );
        endif;
        die();
    }
